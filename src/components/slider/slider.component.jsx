import React from "react";
import Slider from "react-slick";

import "slick-carousel/slick/slick.css";

import { first, second, third, fourth, fifth } from "../../assets/assets.js";
import "./slider.styles.scss";

export default class SlickSlider extends React.Component {
  render() {
    const settings = {
      infinite: true,
      speed: 18000,
      autoplay: true,
      autoplaySpeed: 100,
      slidesToShow: 4,
      slidesToScroll: 1,
      dots: false,
      arrows: false,
      cssEase: "linear",
      swipeToSlide: true,
      centerMode: true,
    };

    return (
      <Slider {...settings} className="funslider">
        <div className="funslider__item">
          <div
            className="funslider__img small"
            style={{
              backgroundImage: `url(${first})`,
            }}
          ></div>
        </div>
        <div className="funslider__item">
          <div
            className="funslider__img large"
            style={{
              backgroundImage: `url(${second})`,
            }}
          ></div>
        </div>
        <div className="funslider__item">
          <div
            className="funslider__img largest"
            style={{
              backgroundImage: `url(${third})`,
            }}
          ></div>
        </div>
        <div className="funslider__item">
          <div
            className="funslider__img large"
            style={{
              backgroundImage: `url(${fourth})`,
            }}
          ></div>
        </div>
        <div className="funslider__item">
          <div
            className="funslider__img small"
            style={{
              backgroundImage: `url(${fifth})`,
            }}
          ></div>
        </div>
      </Slider>
    );
  }
}
